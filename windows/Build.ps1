...
try {
    ...
    # Generate a builder-local strong-name key. Private signing keys are never
    # committed to the public repository.
    $strongNameKey = Join-Path $PSScriptRoot 'Driver\AstroOrbit.LocalServer.snk'
    if (-not (Test-Path -LiteralPath $strongNameKey)) {
        $rsa = New-Object System.Security.Cryptography.RSACryptoServiceProvider 2048
        try { [IO.File]::WriteAllBytes($strongNameKey, $rsa.ExportCspBlob($true)) }
        finally { $rsa.Dispose() }
        Write-Host 'Generated local strong-name signing key.'
    }
