...
foreach ($view in $views) {
    ...
    try {
        ...
        if ($null -eq $cls) { throw "Astro Orbit is not registered in $view. Install setup 1.4.4." }
        $inproc = $cls.OpenSubKey('InprocServer32')
        if ($null -ne $inproc) { throw "Old DLL registration in $view. Install setup 1.4.4 with NINA closed." }
...
try {
    ...
    if ($driver.DriverVersion -ne '1.4.4') { throw 'Old driver is still active. Close all Astro software, install 1.4.4 and retry.' }
